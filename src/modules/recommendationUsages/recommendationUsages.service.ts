...
export const recommendationUsagesService = {
     ...
     */
    async getAll(page: number = 1, limit: number = 10): Promise<{ recommendationUsages: RecommendationUsage[] }> {
     ...
     */
    async getById(id: number): Promise<{ recommendationUsage: RecommendationUsage | null }> {
        const recommendationUsage = await prisma.recommendationUsage.findUnique({ where: { id: id } });
        return { recommendationUsage };
    },
    async create(recommendationUsageData: any): Promise<{ recommendationUsage: RecommendationUsage | null }> {
        const { recommendation_id, order_item_id, confirmed_by, is_rec_by_sales, is_influenced } = recommendationUsageData;

        //Check if recommendation exists
        const recommendation = await prisma.recommendation.findUnique({ where: { id: recommendation_id } });
        if (!recommendation) {
            throw { status: 400, message: 'Recommendation not found' };
        }

        //Check if order item exists
        const orderItem = await prisma.orderItem.findUnique({ where: { unikazon: order_item_id } });
        if (!orderItem) {
